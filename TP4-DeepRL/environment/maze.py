    ...
    def get_dynamics(self, state: tuple[int, int], action: int, next_state: tuple[int, int]):
        ...
        if action == Maze.ACTIONS.index("up"):  # up
            if (self.maze[state[0] - 1, state[1]] == 0) and (next_state == (state[0] - 1, state[1])):  # can go up
                ...
                proba = 1
        elif action == Maze.ACTIONS.index("down"):  # down
            if (self.maze[state[0] + 1, state[1]] == 0) and (next_state == (state[0] + 1, state[1])):  # can go down
                ...
                proba = 1
        elif action == Maze.ACTIONS.index("left"):  # left
