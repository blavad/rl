...
class DQNAgent:
    ...
    def learn(self, env, n_episodes, max_steps):
        ...
        for episode in range(n_episodes):
            ...
            self.epsilon = max(self.final_epsilon, self.epsilon - (1.0 / self.final_exploration_episode))
            ...
            if episode % DQNAgent.TEST_FREQUENCY == DQNAgent.TEST_FREQUENCY - 1:
                ...
                print(
                    "Episode: %5d/%5d, steps: %6d, test success ratio: %.2f, epsilon: %.2f, time: %.1f"
                    % (
                        episode + 1,
                        n_episodes,
                        self.ds,
                        np.sum(test_extra_steps == 0) / 100,
                        self.epsilon,
    ...
    def updateQ(self, state, action, reward, next_state, terminal):
        ...
        if self.ds >= self.replay_memory_size:
            ...
            x_batch, a_batch, r_batch, y_batch, t_batch = (
                torch.from_numpy(self.Ds[c]),
                torch.from_numpy(self.Da[c]),
                torch.from_numpy(self.Dr[c]),
                torch.from_numpy(self.Dsn[c]),
                torch.from_numpy(self.Dt[c]),
            )
            ...
            target_value = self.target_net(y_batch).max(1).values * self.gamma * (1.0 - t_batch) + r_batch
